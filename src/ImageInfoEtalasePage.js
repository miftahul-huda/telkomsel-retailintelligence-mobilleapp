...
export default class ImageInfoEtalasePage extends React.Component {
    constructor(props)
    {
        super(props);
        this.state = {
    ...
    loadFileInfoLocal(callback)
    {
        ...
        UploadedFile.findByPk(this.state.file.id).then(function (file){
            ...
            Logging.log(err, "error", "ImageInfoEtalasePage.loadFileInfo().UploadedFile.findByPk()")
        })        
    }
    ...
    loadLookupData(me, callback)
    {
        me.getOperators().then(function(operators){
            ...
            me.getStores().then(function(stores){
                ...
                Logging.log(e, "error", "ImageInfoEtalasePage.loadLookupData().me.getStores()")
            })
        }).catch((e)=>{
            Logging.log(e, "error", "ImageInfoEtalasePage.loadLookupData().me.getOperators()")
        })
    }

    initEtalaseItemsFromRemote(me)
    {
        console.log("initEtalaseItemsFromRemote")
        ...
        HttpClient.get(url, function(response){
            ...
            console.log("Error");
            console.error(err);
            Logging.log(err, "error", "ImageInfoEtalasePage.initEtalaseItemsFromRemote().HttpClient.get(" + url + " ) ")

        })
    }

    initEtalaseItemsFromLocal(me)
    {
        console.log("initEtalaseItemsFromLocal")
        let tempid = me.getMaxTempId();
        EtalaseItem.findAll({
            where: {
            ...
            console.log("Error");
            console.error(err);
            Logging.log(e, "error", "ImageInfoEtalasePage.initEtalaseItems().EtalaseItem.findAll()")
        })
    }

    initEtalaseItems(me){
        if(this.state.file.uploaded_filename == null || this.state.file.uploaded_filename.length == 0)
            this.initEtalaseItemsFromLocal(this);
        else
            this.initEtalaseItemsFromRemote(this);
    }
    ...
    removeLocalEtalaseItem(item)
    {
        EtalaseItem.destroy({ where: { id: selectedItem.id  }  }).then(function(res){
            me.initEtalaseItems(me);
        });
    }

    removeEtalaseItem(item)
    {
        var me = this;
        var selectedIdx = -1;
        this.state.etalaseItems.forEach(function(it, idx){
            if( (item.id != null && item.id == it.id) || ( item.tempid == it.tempid )){
                selectedIdx = idx;
            }
        })

        var etalaseItems = this.state.etalaseItems;

        if(selectedIdx > -1)
        {
            var selectedItem = this.state.etalaseItems[selectedIdx];
            if(selectedItem.id != null)
            {
                if(me.state.file.uploaded_filename == null)
                    me.removeLocalEtalaseItem();
                else {
                    me.state.etalaseItems.splice(selectedIdx, 1);
                    me.setState({
                        ...me.state,
                        etalaseItems: me.state.etalaseItems
                    })
                }
            }
            else
    ...
    {
        if(this.state.etalaseItems.length == 0)
    ...
    uploadImageInfo(me, uploaded_filename, item, callback, callbackError){
        ...
        HttpClient.post(url, newFile, function (res){
            if(res.success){
                ...
                me.uploadEtalaseItems(me, me.state.etalaseItems,0, res.payload.id, function(){
                    if(callback != null)
                        callback(res);
                }, function(res){
                    res.message = " uploadetalaseItems : " + res.message;
                    if(callbackError != null)
                        callbackError(res);
                });
            ...
            Logging.log(res, "error", "ImageInfoEtalasePage.uploadImageInfo().HttpClient.post()")
        });
    }

    uploadEtalaseItems(me, items, idx, parentid, callback, callbackError)
    {
        if(idx < items.length)
        {
            items[idx].upload_file_id = parentid;
            me.uploadEtalaseItem(me, items[idx], function(res){
                me.uploadEtalaseItems(me, items, idx + 1, parentid, callback, callbackError);  
            }, function(res){
                if(callbackError != null)
                    callbackError(res);
            });
    ...
    uploadEtalaseItem(me, item, callback, callbackError)
    {
        ...
        HttpClient.post(url, newItem, function (res){
            ...
            {
                res.message = " uploadEtalaseItem : " + res.message;
                if(callbackError != null)
                    callbackError(res);
            }
        }, function(err){
            Logging.log(err, "error", "ImageInfoEtalasePage.uploadEtalaseItem().HttpClient.post()")
        });
    }
    ...
    uploadToServer(callback, callbackError)
    {
        ...
        HttpClient.upload(url, this.state.file.filename, function(res){
            ...
            console.log(res);
            Logging.log(res, "error", "ImageInfoEtalasePage.uploadToServer().HttpClient.upload()")
        });
    ...
    updateUploadedFileLocally(me, callback, callbackError)
    {
        ...
        UploadedFile.update({ isuploaded : 1, uploaded_id: dt }, {
            ...
            if(callbackError != null)
                callbackError(err);
            Logging.log(err, "error", "ImageInfoEtalasePage.updateUploadedFileLocally().UploadedFile.update()")
        });
    }

    saveLocalUpdateEtalaseItem(items, idx, callback)
    {
        var me = this;
        if(idx < items.length)
        {
            
            EtalaseItem.update({ operator : items[idx].operator, percentage: items[idx].percentage, productHero: items[idx].productHero, 
                ...
                {where: { id: items[idx].id }}).then(function (res){
                    ...
                    me.saveLocalUpdateEtalaseItem(items, idx + 1, callback);
                    return null;

            }).catch(err => {
                console.log("ImageInfoEtalasePage.saveLocalUpdateEtalaseItem().EtalaseItem.update()")
                console.log(err)
                Logging.log(err, "error", "ImageInfoEtalasePage.saveLocalUpdateEtalaseItem().EtalaseItem.update()")
            })
    ...
    saveLocalDeleteEtalaseItem(items, idx, callback)
    {
        var me = this;
        if(idx < items.length)
        {
            EtalaseItem.destroy({where: { id: items[idx].id }}).then(function (res){
                me.saveLocalDeleteEtalaseItem(items, idx + 1, callback);
            }).catch((err)=>{
                Logging.log(err, "error", "ImageInfoEtalasePage.saveLocalUpdateEtalaseItem().EtalaseItem.destroy()")
            })
        }
        else
            if(callback != null)
    ...
    saveLocalUploadedFileInfo(callback)
    {
        ...
        this.state.etalaseItems.forEach(function (item){
        ...
        UploadedFile.update({ store_name : this.state.file.store_name, operatorDominant: this.state.file.operatorDominant }, {
          ...
          console.log(createdItems)
           ...
           EtalaseItem.bulkCreate(createdItems).then(function (res){
               console.log("save created!")
                ...
                me.saveLocalUpdateEtalaseItem( updatedItems, 0, function(){
               ...
               console.log(err);
               Logging.log(err, "error", "ImageInfoEtalasePage.saveLocalUploadedFileInfo().UploadedFile.update()")
           })

           return true;
        }).catch(function (err){
            console.log(err);
            Logging.log(err, "error", "ImageInfoEtalasePage.saveLocalUploadedFileInfo().EtalaseItem.bulkCreate()")
        })
    }
        ...
        HttpClient.post(url, newFile, function (res){
            if(res.success){
                ...
                me.deleteRemotePackageItems(function(response){
                    ...
                    me.uploadEtalaseItems(me, me.state.etalaseItems,0, me.state.file.id, function(){
                        me.setState({
                            showIndicator: false
                        })
                        ...
                        Logging.log(res, "error", "ImageInfoEtalasePage.saveUpdateRemote().uploadEtalaseItems()")
                        if(callbackError != null)
                            callbackError(res);
                    });
                    ...
                    Logging.log(err, "error", "ImageInfoEtalasePage.saveUpdateRemote().deleteRemotePackageItems()")
                    if(callbackError != null)
                        callbackError(err);
                })
            ...
            else
            {
                ...
                Logging.log(res, "error", "ImageInfoEtalasePage.saveUpdateRemote().HttpClient.post()")
    ...
    editPackageItem(item)
    {
        ...
        Actions.editEtalaseItemPage({ mode: "edit", item: item, onSave: this.onSaveEtalaseItem.bind(this), operator: this.state.selectedOperator });
    }
    ...
    addEtalaseItem()
    {
        ...
        Actions.editEtalaseItemPage({ mode: "add", item: item, onSave: this.onSaveEtalaseItem.bind(this), operator: this.state.selectedOperator });
    }
    ...
    onSaveEtalaseItem(item)
    {
        ...
        let exists = false;
        for(let i =0; i < this.state.etalaseItems.length; i++)
        {
            if(this.state.etalaseItems[i].tempid == item.tempid)
            {
                this.state.etalaseItems[i] = item;
                exists = true;
            }
        }
        ...
        if(exists == false)
        {
            this.state.etalaseItems.push(item);
        ...
        this.setState({ 
            ...this.state,
            etalaseItems: this.state.etalaseItems   
        });
    }
        ...
        return(
            ...
            <ImageBackground source={require('./images/background.png')} style={{ width: '100%', height: '100%' }}>
                <View style={{ flex: 1, height: '100%', padding: '5%' }}>
                        ...
                        <List>
                            {
                                this.state.etalaseItems.map((item, idx) => {
                                    return(<ListItem key={idx}>
                                        ...
                                        <TouchableOpacity onPress={this.removeEtalaseItem.bind(this, item)} style={{marginLeft: '0%', padding: '0%'}} >
                        ...
                        { (this.props.mode == "edit" ) ?
                        <View style={{ flex:1, flexDirection: 'row'}} >
                            <TouchableOpacity onPress={this.addEtalaseItem.bind(this)} style={{marginTop: '0%', padding: '3%'}} >
